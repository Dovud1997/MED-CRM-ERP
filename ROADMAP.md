...
- Реализована электронная очередь и ТВ-экраны: `queue_entries` поверх пациентов/записей/врачей/филиалов, транзакционная выдача номера, публичный экран `/display/{token}` с SSE (Redis Pub/Sub), медиатека в отдельном volume, режим конфиденциальности `NUMBER_ONLY`, `queue_call_history` и аудит (см. `docs/deployment/ELECTRONIC-QUEUE.md`).
...
- Go worker (Redis-backed), SMS/email/Telegram notifications, outbox pattern.
